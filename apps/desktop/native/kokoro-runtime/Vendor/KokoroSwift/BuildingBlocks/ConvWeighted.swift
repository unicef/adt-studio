...
class ConvWeighted: Module {
  ...
  public func callAsFunction(_ x: MLXArray, conv: (MLXArray, MLXArray, Int, Int, Int, Int, Int, StreamOrDevice) -> MLXArray) -> MLXArray {
    ...
    if groups > 1 {
      weightToUse = weight
    } else if weight.ndim == 3, let inputChannels = x.shape.last, weight.shape[0] == inputChannels {
      weightToUse = weight.transposed(2, 1, 0)
    } else if x.shape.last == weight.shape.last {
      weightToUse = weight
    } else {
      weightToUse = weight.transposed()
    }

    return applyConv(x: x, weightToUse: weightToUse)
  }
}
