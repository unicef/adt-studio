...
public final class KokoroTTS {
  ...
  public init(modelPath: URL, configPath: URL, g2p: G2P? = nil) throws {
    let sanitizedWeights = WeightLoader.loadWeights(modelPath: modelPath)
    let config = try KokoroConfig.loadConfig(from: configPath)
    self.config = config
  ...
  public func generateAudioFromPhonemes(voice: MLXArray, phonemes: String, speed: Float = 1.0) throws -> [Float] {
    BenchmarkTimer.reset()
    BenchmarkTimer.startTimer(Constants.bm_TTS)

    let (paddedInputIds, attentionMask, inputLengths, textMask, inputIds) = try prepareInputTensors(phonemes)
    let (globalStyle, acousticStyle) = extractStyleEmbeddings(from: voice, tokenCount: inputIds.count)
    let durationFeatures = encodeBERTAndDuration(
      inputIds: paddedInputIds,
      attentionMask: attentionMask,
  ...
  private func extractStyleEmbeddings(from voice: MLXArray, tokenCount: Int) -> (MLXArray, MLXArray) {
    var referenceStyle = voice[tokenCount - 1]
    if referenceStyle.ndim == 1 {
      referenceStyle = referenceStyle.expandedDimensions(axis: 0)
    }

    let globalStyle = referenceStyle[0..., 128...]
    let acousticStyle = referenceStyle[0..., 0 ..< 128]
