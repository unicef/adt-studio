...
export default { async fetch(request:Request,env:Env):Promise<Response>{
  ...
  try { const url=new URL(request.url),repo=new ClassroomRepository(env.DB),service=new ClassroomService(repo,env.DB),studentId=idFrom(url,/^\/students\/([^/]+)$/),materialId=idFrom(url,/^\/materials\/([^/]+)$/)
