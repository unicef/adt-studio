...
function StudentPage() {
  ...
  const handleGenerate = () => generate.mutate(
    { studentId, sourceBookLabel: bookLabel },
    {
      onSuccess: async (result) => {
        try {
          ...
          const materialTitle = `${student.accessibilityProfiles[0]?.name ?? student.firstName} material`
  ...
  return <div className="flex min-h-0 flex-1 flex-col"><StudioTopBar brandLinksHome trailingTitle={<Trans>Student profile</Trans>} /><main className="mx-auto w-full max-w-5xl space-y-8 overflow-auto p-6">
    ...
    <section className="grid gap-6 lg:grid-cols-2"><div className="rounded-lg border p-5"><h2 className="mb-3 text-lg font-semibold"><Trans>Accessibility profiles</Trans></h2><div className="space-y-2">{student.accessibilityProfiles.map((profile) => <div key={profile.id} className="flex items-start justify-between gap-3 rounded border p-3"><div><p className="font-medium">{profile.name}</p>{profile.description && <p className="text-sm text-muted-foreground">{profile.description}</p>}</div><Button variant="ghost" size="icon" aria-label={t`Delete profile`} title={t`Delete profile`} disabled={deleteProfile.isPending} onClick={() => deleteProfile.mutate(profile.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div>)}{student.accessibilityProfiles.length === 0 && <p className="text-sm text-muted-foreground"><Trans>No profiles added yet.</Trans></p>}</div></div>
