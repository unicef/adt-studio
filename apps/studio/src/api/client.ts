...
export function getSignLanguageVideoUrl(
  label: string,
  videoId: string,
): string {
...
export interface ImportPreview {
  ...
  stages: Record<
    string,
    { status: string; stepCount: number; doneCount: number }
  >;
...
export interface TextCatalogResponse {
  ...
  translations: Record<
    string,
    { entries: TextCatalogEntry[]; version: number }
  >;
  speechTexts: Record<
    string,
    { entries: CoreTtsCatalogEntry[]; version: number }
  >;
}
...
export function getBookFontFileUrl(
  label: string,
  fontId: string,
  file: string,
): string {
...
export const api = {
  ...
  getBookFonts: (label: string) =>
    request<BookFontsResponse>(`/books/${label}/fonts`),
  ...
  generateEditableActivityFeedback: (
    ...
    request<{
      feedback: Record<string, { correct?: string; incorrect?: string }>;
    }>(
  ...
  applyBookFont: (label: string, payload: ApplyBookFontPayload) =>
    request<BookFontsResponse & { pagesUpdated: number }>(
      `/books/${label}/fonts/apply`,
      {
        method: "POST",
  ...
  updateImageClassification: (label: string, pageId: string, data: unknown) =>
    request<{ version: number }>(
      `/books/${label}/pages/${pageId}/image-filtering`,
      {
        method: "PUT",
        body: JSON.stringify(data),
      },
  ...
  updateImageCaptioning: (label: string, pageId: string, data: unknown) =>
    request<{ version: number }>(
      `/books/${label}/pages/${pageId}/image-captioning`,
      {
        method: "PUT",
        body: JSON.stringify(data),
      },
  ...
  reRenderPage: (
    ...
    request<{
      ...
      status?: string;
      version?: number;
      rendering?: { sections: SectionRendering[] };
    }>(
  ...
  getAccessibilityAssessment: (label: string) =>
    request<AccessibilityAssessmentResponse>(
      `/books/${label}/debug/accessibility`,
    ),

  getReviewerValidationCatalog: (label: string) =>
    request<ReviewerValidationCatalogResponse>(
      `/books/${label}/validation/catalog`,
    ),

  getReviewerValidationSessions: (label: string) =>
    request<ReviewerValidationSessionsResponse>(
      `/books/${label}/validation/sessions`,
    ),
  ...
  saveReviewerValidationSession: (
    ...
    request<ReviewerValidationSessionEntry>(
      `/books/${label}/validation/sessions`,
      {
        method: "POST",
  ...
  saveReviewerPageValidationRecord: (
    ...
    request<ReviewerPageValidationRecordEntry>(
      `/books/${label}/validation/page-results`,
      {
        method: "POST",
  ...
  getPrompt: (name: string, bookLabel?: string, modelId?: string | null) => {
    ...
    return request<PromptResponse>(
      bookLabel
        ? `/books/${bookLabel}/prompts/${name}${query}`
        : `/prompts/${name}${query}`,
    );
  },
  ...
  listPromptVersions: (
    ...
    return request<PromptVersionsResponse>(
      bookLabel
        ? `/books/${bookLabel}/prompts/${name}/versions${query}`
        : `/prompts/${name}/versions${query}`,
    );
  },
  ...
  getTemplate: (name: string, bookLabel?: string) =>
    request<{ name: string; content: string; source?: string }>(
      bookLabel
        ? `/books/${bookLabel}/templates/${name}`
        : `/templates/${name}`,
    ),
  ...
  updateEasyRead: (
    label: string,
    data: { blocks: EasyReadSectionBlock[]; generatedAt: string },
  ) =>
  ...
  updateTranslation: (label: string, language: string, data: unknown) =>
    request<{ version: number }>(
      `/books/${label}/text-catalog-translation/${language}`,
      {
        method: "PUT",
        body: JSON.stringify(data),
      },
  ...
  getTranslationEvaluations: (label: string) =>
    request<TranslationEvaluationStatusesResponse>(
      `/books/${label}/evaluations/translations`,
    ),

  getTranslationEvaluation: (label: string, language: string) =>
    request<TranslationEvaluationStatusResponse>(
      `/books/${label}/evaluations/translations/${language}`,
    ),
  ...
  runTranslationEvaluation: (
    ...
    request<TranslationEvaluationRunResponse>(
      `/books/${label}/evaluations/translations/${language}/run`,
      {
        method: "POST",
        headers: buildApiHeaders(apiKey, providerCredentials),
        body: JSON.stringify({
          ...(scope.pageId ? { page_id: scope.pageId } : {}),
          ...(scope.entryIds && scope.entryIds.length > 0
            ? { entry_ids: scope.entryIds }
            : {}),
        }),
      },
    ),

  acceptTranslationEvaluationItemAnyway: (
    label: string,
    language: string,
    entryId: string,
  ) =>
  ...
  getTTS: (label: string) => request<TTSResponse>(`/books/${label}/tts`),
    ...
    request<GenerateSingleTTSResponse>(`/books/${label}/tts/generate-one`, {
      ...
      headers: {
        ...
        ...(credentials.azure?.key
          ? { "X-Azure-Speech-Key": credentials.azure.key }
          : {}),
        ...(credentials.azure?.region
          ? { "X-Azure-Speech-Region": credentials.azure.region }
          : {}),
        ...(credentials.elevenLabsApiKey
          ? { "X-ElevenLabs-API-Key": credentials.elevenLabsApiKey }
          : {}),
      },
  ...
  uploadTTSForItem: (
    ...
    return request<GenerateSingleTTSResponse>(
      `/books/${label}/tts/upload-one`,
      {
        method: "POST",
        body: formData,
      },
  ...
  getWordTimestamps: (label: string, language: string) =>
    request<WordTimestampResponse>(
      `/books/${label}/tts/timestamps/${language}`,
    ),
  ...
  transcribeOne: (
    ...
    request<{ entry: WordTimestampEntry }>(
      `/books/${label}/tts/transcribe-one`,
      {
        method: "POST",
        headers: {
  ...
  saveWordTimestamps: (
    ...
    language: string,
    textId: string,
    data: { words: WordTimestamp[]; duration: number },
  ) =>
    request<{ ok: boolean }>(
      `/books/${label}/tts/timestamps/${language}/${textId}`,
      {
        method: "PUT",
        body: JSON.stringify(data),
      },
  ...
  transcribeAll: (label: string, language: string, openaiApiKey: string) =>
    request<{ taskId: string | null; count?: number; skipped?: number }>(
      `/books/${label}/tts/transcribe-all`,
      {
        method: "POST",
        headers: {
          "X-OpenAI-Key": openaiApiKey,
  ...
  getTemplates: () => request<{ templates: string[] }>(`/templates`),

  getStyleguides: (bookLabel?: string) =>
    request<{ styleguides: string[] }>(
      bookLabel
        ? `/styleguides?book=${encodeURIComponent(bookLabel)}`
        : `/styleguides`,
    ),
  ...
  getSignLanguageVideos: (label: string) =>
    request<{ videos: SignLanguageVideo[] }>(
  ...
  assignSignLanguageVideo: (
    label: string,
    videoId: string,
    sectionId: string | null,
  ) =>
    request<{ ok: boolean }>(
      `/books/${label}/sign-language-videos/${videoId}/assign`,
      {
        method: "PUT",
  ...
  deleteSignLanguageVideo: (label: string, videoId: string) =>
    request<{ ok: boolean }>(
      `/books/${label}/sign-language-videos/${videoId}`,
      {
        method: "DELETE",
      },
    ),
  ...
  stopLocalAI: () =>
    request<{ state: LocalAIStatus["state"] }>(`/local-ai/stop`, {
      method: "POST",
    }),

  getLocalSpeechStatus: () =>
    request<LocalSpeechStatus>(`/local-speech/status`),

  searchLocalSpeechModels: (query: string) =>
    request<Array<{ id: string; compatible: boolean; mlxCompatible: boolean }>>(
      `/local-speech/search?q=${encodeURIComponent(query)}`,
    ),

  installLocalSpeechModel: (
    repository: string,
    voice: string,
    runtime: "onnx" | "mlx" = "onnx",
  ) =>
  ...
  removeLocalSpeechModel: (repository: string) =>
    request<{ removed: string }>(`/local-speech/models/${repository}`, {
      method: "DELETE",
    }),

  testLocalSpeechModel: async (
    repository: string,
    voice: string,
    text: string,
  ): Promise<Blob> => {
  ...
  getDefaultModel: () => request<{ model: string }>(`/config/default-model`),
