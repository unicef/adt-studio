...
export function StoryboardCommentsSidebar({
  ...
  /** The same label the overlay draws, so a row and its dot cannot disagree about who wrote it. */
  const pinLabels = useMemo(() => {
    const labels = new Map<string, string>()
    for (const group of groups) {
      for (const thread of group.threads) labels.set(thread.root.id, initialOf(thread.root.author_name))
    }
    return labels
  }, [groups])
          ...
          {!published ? (
            ...
            groups.map((group) => (
              <section key={group.sectionId} className="border-b last:border-b-0">
                ...
                {/* ThreadRow renders its own `li`, so it goes straight in the list — wrapping
                    it in another one nests `li` inside `li`, which React reports as a
                    hydration error on every render of this panel. */}
