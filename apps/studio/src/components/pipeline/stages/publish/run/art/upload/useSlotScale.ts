 ...
 *
 * The alternative — laying the drawing out responsively — sounds better and is worse here. This is
 * a scene whose depth is baked: a belt authored against one width, a machine whose housing has to
 * swallow a plate whole. Re-deriving that at every card size would mean several illustrations, and
 * only one of them would ever get the attention. So the composition is authored once and one
 * `scale()` on one node fits it to whatever the card gives it.
 *
 * `max` caps the growth: a 400-wide composition blown up further gains nothing and loses crispness
 * on its baked gradients. `mode: "cover"` is for a scene whose subject is an *atmosphere* rather
