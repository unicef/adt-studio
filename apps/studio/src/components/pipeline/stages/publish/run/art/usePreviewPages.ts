...
import { getBookCoverUrl, getPageImageUrl } from "@/api/client"
...
export function usePreviewPages(label: string | null, count = 8): readonly string[] {
  ...
  return useMemo(() => {
    if (!label) return []
    const bookPages = pages.data ?? []
    if (bookPages.length === 0) return [getBookCoverUrl(label)]
    const stride = Math.max(1, Math.floor(bookPages.length / count))
    return bookPages
      .filter((_page, index) => index % stride === 0)
      .slice(0, count)
      .map((page) => getPageImageUrl(label, page.pageId))
