...
import { useSlotScale } from "../upload/useSlotScale"
...
export function FitBox({
  ...
  /** `max: Infinity` keeps this art's long-standing behaviour: unlike the upload scene, which
   *  caps at 1.25 because its baked gradients soften when blown up, the sheet feed has always
   *  been free to grow into whatever the band gives it. Whether the two should agree is a design
   *  question, not something to settle inside a deduplication. */
  const { ref, scale } = useSlotScale({ width, height }, { max: Infinity })
