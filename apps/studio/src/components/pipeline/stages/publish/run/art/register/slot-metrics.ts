 ...
 *
 * The card this sits in is anywhere from a narrow panel to a full-width page, and the two ways of
 * surviving that range are both wrong on their own. Scaling one fixed 320×200 drawing down by half takes
 ...
 * drawn as a book-shaped rectangle rather than the book. That is gone; the spread fits its pages
 * from the book's real trim in `reader-metrics.ts`. What is left is only the row, which is
 * genuinely relative to the space it is given.
 */
