...
export function usePageAspect(pages: readonly string[]): number {
  ...
  useEffect(() => {
    ...
    /* `decode()` only exists in a real browser; jsdom and older Safari answer through the load
     * event instead, a beat earlier but with the same naturalWidth. */
    const ready =
      typeof image.decode === "function"
        ...
        : new Promise<void>((resolve, reject) => {
            ...
            image.addEventListener("error", () =>
              reject(new Error("page image failed to load")),
            )
          })
