...
/** Ambient page units on a stride. Spread books keep each merged facing pair together: the cover
 * stays single-width, while each later unit is twice as wide at the same page height. */
interface BeltItem {
  slot: PageSlot
  wide: boolean
}
...
export function AssemblyLine({ pages, progress, aspect, spread, className }: StepAnimationProps) {
  ...
  /* The cover is one page; each merged spread is twice as wide, never taller. */
  const pageH = 44
  const plateW = (wide: boolean) => Math.min(84, pageH * clamp(wide ? aspect * 2 : aspect, 0.5, 2))
