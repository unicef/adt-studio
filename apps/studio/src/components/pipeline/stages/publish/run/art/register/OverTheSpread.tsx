...
function OverTheSpreadBody({
  ...
  // A completed publication holds one clear opening. The cover remains a standalone leaf;
  // every spread render is split into two same-height pages before it reaches this scene.
  const first = pairs[0]
    ...
    <div ref={bodyRef} className="pubreg-i-body" style={metrics as CSSProperties}>
      ...
      <div className="pubreg-i-art" aria-hidden="true">
        <span className="pubreg-i-spread">
          {spread ? (
            <span className="absolute inset-0 overflow-hidden rounded-[6px] bg-card shadow-[0_5px_13px_-8px_rgba(15,23,42,0.45)]">
              <Leaf src={pages[1] ?? pages[0]} seed={0} />
            </span>
          ) : (
            <>
              <span className="pubreg-i-leaf pubreg-i-leaf--verso">
                <Leaf src={first?.verso?.src} half={first?.verso?.half} seed={0} />
              </span>
              <span className="pubreg-i-leaf pubreg-i-leaf--recto">
                <Leaf src={first?.recto?.src} half={first?.recto?.half} seed={1} />
              </span>
            </>
          )}
