...
import { toBookVM, type CoverSpec } from "@/components/redesign/shared/data"
...
import { rankBySearch, searchTokens } from "@/components/redesign/shared/search"
...
function PaletteResults({ onClose, books, locale, onOpenAdd }: PaletteResultsProps) {
  ...
  const groups = useMemo<PaletteGroup[]>(() => {
    ...
    const nav: PaletteItem[] = [
      ...
      { id: "nav-handoffs", title: t`Split & merge`, icon: Split, run: () => navigate({ to: REDESIGN_PATHS.handoffs }) },
      { id: "nav-settings", title: t`Settings`, icon: Settings, run: () => navigate({ to: REDESIGN_PATHS.settings }) },
    ]
