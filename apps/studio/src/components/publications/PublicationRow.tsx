...
/** `grid-rows` 0fr→1fr is the one height transition that needs no measuring, and
 *  `transition-discrete` holds `visibility` until it finishes, so a shut panel keeps its
 *  buttons out of the tab order without blinking on the way down. Always in the DOM, collapsed:
 *  mounting and opening in the same commit gives the browser no start value to interpolate
 *  from, which is why a mount-on-open version animated shut but never open. */
function Collapse({
  id,
  open,
  children,
...
export function PublicationRow({
  ...
  return (
    <li
      ...
      className="rounded-xl border bg-card transition-[border-color,box-shadow] duration-200 hover:border-primary/30 hover:shadow-sm motion-safe:animate-wizard-enter motion-reduce:transition-none dark:hover:shadow-black/30"
    >
      <article aria-labelledby={titleId} className="flex flex-col gap-3 p-4">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {publication.book_exists ? null : (
                <span className="inline-flex w-fit shrink-0 items-center gap-1.5 rounded-full border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[11px] font-medium text-red-800 dark:text-red-300">
            ...
            {url ? (
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                >
                  {copied ? (
                    <Check
                      className="size-3.5 text-emerald-600 dark:text-emerald-400 motion-safe:animate-in motion-safe:zoom-in-50 motion-safe:duration-200"
                      aria-hidden="true"
                    />
                  ) : (
            ...
            {copied || copyFailed ? (
              <span role="status" aria-live="polite" className="text-xs leading-5 text-emerald-700 dark:text-emerald-400">
        ...
        {/* One toolbar under the details rather than a column beside them: five stacked
            buttons made every card three times taller than its own content, so a shelf of
            four books needed a scroll to see the second. */}
        <div className="flex flex-wrap items-center gap-1.5 border-t pt-3">
          ...
          >
            ...
            {countsKnown && publication.unresolved_count > 0 ? (
              <span className="rounded-full bg-primary px-1.5 text-[10px] font-bold leading-4 text-primary-foreground tabular-nums">
          ...
          <span className="ml-auto flex flex-wrap items-center gap-1.5">
            <Button
              ...
              className={cn(
                "h-8 gap-2 text-xs",
                state === "revoked" ? "text-emerald-700 dark:text-emerald-400" : "text-muted-foreground",
              )}
            ...
            <Button
              ...
              className={cn(
                "h-8 gap-2 text-xs transition-colors duration-200 motion-reduce:transition-none",
        ...
        {/* Grown open rather than swapped in. Replacing the button with the warning made the
            ...
            misclick. */}
        <Collapse id={confirmId} open={confirmingDelete}>
          <div
          ...
          >
            <p className="min-w-0 flex-1 text-xs leading-5 text-red-900 dark:text-red-200">
              <Trans>
            ...
            <div className="flex shrink-0 gap-1.5">
              <Button
                ...
                variant="destructive"
                size="sm"
                className="h-7 text-xs transition-transform duration-200 active:scale-[0.98] motion-reduce:transition-none"
                disabled={deleting}
                onClick={onDelete}
              >
                {deleting ? (
            ...
            {deleteError ? (
              <div
                ...
                role="alert"
                className="flex w-full flex-wrap items-center gap-2 border-t border-destructive/30 pt-2 duration-200 motion-safe:animate-in motion-safe:fade-in-0"
              >
      ...
      {/* A drawer under the whole card: it spans the divider. Cost of always rendering it is
          nothing: `enabled` is what gates the request, so a shelf of forty books still fetches
          no rosters until one is asked for. */}
      <Collapse id={readersId} open={readersOpen}>
