...
export function PublishingSettings() {
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col gap-5">
      <header>
        <h1 className="mb-1 text-2xl font-bold tracking-[-0.02em] text-foreground"><Trans>Publishing</Trans></h1>
      ...
      {hasConnectionHint && connection.isError && (
        <div data-testid="connection-check-error" className="flex flex-col gap-2 rounded-xl border border-amber-500/40 bg-amber-500/5 p-4">
          <span className="flex items-center gap-2 text-sm font-medium text-foreground"><AlertTriangle className="size-4 shrink-0 text-amber-600 dark:text-amber-400" aria-hidden="true" /><Trans>We couldn't check your publishing setup</Trans></span>
