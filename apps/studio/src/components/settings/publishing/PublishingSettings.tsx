...
export function PublishingSettings() {
  ...
  return (
    <div className="mx-auto flex min-h-0 w-full flex-1 flex-col gap-6 p-5">
      <header>
        <h1 className="text-xl font-semibold tracking-tight text-foreground"><Trans>Publishing</Trans></h1>
        <p className="mt-1.5 max-w-2xl text-sm leading-6 text-muted-foreground"><Trans>Connect a Cloudflare account to host finished books as private websites.</Trans></p>
      </header>
      {hasConnectionHint && connection.isError && (
        <div data-testid="connection-check-error" className="flex flex-col gap-2 rounded-xl border border-amber-500/40 bg-amber-500/5 p-4">
          <span className="flex items-center gap-2 text-sm font-medium text-foreground"><AlertTriangle className="size-4 shrink-0 text-amber-600" aria-hidden="true" /><Trans>We couldn't check your publishing setup</Trans></span>
      ...
      {isChecking && <div className="flex items-center gap-2.5 rounded-xl border bg-card p-4 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin motion-reduce:animate-none" aria-hidden="true" /><Trans>Checking your publishing setup…</Trans></div>}
