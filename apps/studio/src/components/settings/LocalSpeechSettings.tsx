...
export function LocalSpeechSettings() {
  ...
  const install = useMutation({
    mutationFn: () => api.installLocalSpeechModel(repository, voice, runtime),
    ...
    <section className="space-y-4 rounded-lg border p-4">
      ...
      {status.data?.installed.map((model) => (
        <div key={model.repository} className="flex items-center justify-between gap-3 rounded-md bg-muted/40 px-3 py-2 text-sm">
          <div><div className="font-medium">{model.repository}</div><div className="text-xs text-muted-foreground">{model.runtime.toUpperCase()} · {model.dtype} · {model.voices.join(", ")} · {model.revision.slice(0, 8)}</div></div>
