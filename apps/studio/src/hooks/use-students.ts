...
export function useSaveStudent(id?: string) {
  ...
  return useMutation({
    ...
    onSuccess: (student) => { queryClient.invalidateQueries({ queryKey: ["students"] }); queryClient.invalidateQueries({ queryKey: ["student-dashboard"] }); queryClient.setQueryData(["students", student.id], student) },
  })
}
...
export function useAddStudentProfile(studentId: string) {
  const queryClient = useQueryClient()
  return useMutation({ mutationFn: (data: AccessibilityProfileInput) => api.addStudentProfile(studentId, data), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["students", studentId] }) })
}
