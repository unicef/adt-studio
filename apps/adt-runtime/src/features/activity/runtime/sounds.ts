...
/**
 * `page_turn` mirrors the shared ADT reader, which plays this clip at 0.4.
 */
...
/**
 * How long the cue is given before a navigation tears the document down.
 *
 * Matches `DEFAULT_PAGE_TURN_MIN_DURATION_MS` in the shared ADT reader, so a
 * page turn here sounds exactly like a page turn there.
 */
export const PAGE_TURN_LEAD_MS = 350
...
/** Fetches a clip up front so the first play is not waiting on the network. */
