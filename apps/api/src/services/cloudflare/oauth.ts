...
const CLOUDFLARE_OAUTH_REVOKE_URL = "https://dash.cloudflare.com/oauth2/revoke"
export const CLOUDFLARE_OAUTH_CLIENT_ID = "54d11594-84e4-41aa-b438-e81b8fa78ee7"
const CLOUDFLARE_OAUTH_CALLBACK_HOST = "127.0.0.1"
...
function buildAuthorizationUrl(input: AuthorizationUrlInput): string {
...
const OAUTH_SUCCESS_PAGE = page(
  "Connected to Cloudflare",
  "You can close this tab and return to ADT Studio.",
  "#16a34a",
)

function oauthErrorPage(detail: string): string {
...
const defaultListenerFactory: OAuthCallbackListenerFactory = async ({ port, host, handle }) => {
  const server = http.createServer((request, response) => {
    /** `void (async () => ...)()` used to leave the request unprotected: any throw inside —
     *  `handle` failing outside its own try/catch, or a write to an already-closed socket —
     *  became an unhandled rejection that can bring the whole API process down, and if it
     *  happened before `response.end` the browser's tab would just hang waiting on it. The
     *  try/catch keeps the rejection local and answers a best-effort `500` instead; the finally
     *  is the backstop that guarantees the socket is closed even if that `500` itself could not
     *  be written. */
    void (async () => {
      try {
        ...
        try {
          ...
          response.end(
            oauthErrorPage(`Something went wrong finishing the login: ${describeError(error)}`),
          )
        } catch {
          /** The response itself is what failed to write — the finally below is what still
           *  closes the socket. */
        }
      } finally {
        if (!response.writableEnded) {
          try {
            ...
            /** Nothing left to do — the socket is in a state neither branch could write to. */
          }
        }
      }
    })()
...
export function createCloudflareOAuthService(
  ...
  /**
   * Tears down a flow's callback listener. Never rejects.
   *
   * Half of the call sites are `void releaseListener(flow)` — fire-and-forget cleanup from
   * `expireIfStale` and `pruneFlows`, which are synchronous and have nowhere to send a
   * rejection — and the rest `await` it from inside the callback handler itself. Either way, a
   * `listener.close()` that rejects must not become an unhandled rejection or interrupt the
   * caller's own error handling; it is swallowed here, once, so every call site is safe by
   * construction rather than by remembering a `.catch()` at each of them. The logged message is
   * `describeError`'s own text, which describes a socket teardown failure, never a token or the
   * management secret.
   */
  async function releaseListener(flow: PendingFlow): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[cloudflare-oauth] callback listener failed to close: ${describeError(error)}`,
      )
    }
  }
