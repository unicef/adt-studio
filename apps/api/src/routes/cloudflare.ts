...
export function createCloudflareRoutes(deps: CloudflareRoutesDeps): Hono {
  ...
  /** Guards `/cloudflare/provision` against a second concurrent run.
   *
   *  Two overlapping calls each mint their own management secret — `provisionCloudflare` only
   *  reuses `existing.mgmt_secret` when the connection store already has one, so two runs that
   *  start before either has written the record both generate a fresh one and both deploy it to
   *  the worker. Whichever run's `store.write` lands last is the one whose secret the connection
   *  record remembers; the *other* run's secret is what actually shipped if it deployed after
  ...
  app.post("/cloudflare/provision", async (c) => {
    if (provisionInFlight) {
      throw new HTTPException(409, {
        message: "A Cloudflare provision is already running. Wait for it to finish and try again.",
      })
    }
