...
describe("cloudflare routes", () => {
  ...
  /** Lets a test hold a provision run open until it decides to let it proceed, so a second
   *  request can be fired while the first is provably still running. */
  ...
  /** Polls with real timer ticks rather than a fixed number of microtask flushes — see the same
   *  helper in publications.test.ts for why. */
  ...
  describe("POST /cloudflare/provision", () => {
    ...
    /** The race this guards against: two overlapping runs each mint their own management
     *  secret, and whichever run's `store.write` lands last is the one the connection record
     *  remembers — if the *other* run's secret is what actually shipped, every later call
     *  authenticates with the wrong one and gets a permanent 401. The first run is held on its
     *  first network call so a second one can prove it was refused while the first was still
     *  running. */
    it("answers 409 for a second provision while the first is still running", async () => {
      ...
      const firstRequest = app.request("/api/cloudflare/provision", { method: "POST", headers: AUTH })
      await waitUntil(() => started)

      const second = await app.request("/api/cloudflare/provision", { method: "POST", headers: AUTH })
  ...
  describe("DELETE /cloudflare/connection", () => {
    ...
    it("empties a populated bucket before deleting it", async () => {
      const store = createConnectionStore(stateDir)
      store.write(record)
      const snapshotKeys = Array.from(
        { length: 1001 },
        (_, index) => `books/${String(index).padStart(4, "0")}/index.html`,
      )
      const { app, fake } = buildApp({
        databases: [{ uuid: "db-uuid-1", name: "adt-publish" }],
        buckets: ["adt-publish-snapshots"],
        bucketObjects: {
          "adt-publish-snapshots": snapshotKeys,
        },
        scripts: [CLOUDFLARE_WORKER_NAME],
      })
      ...
      expect(res.status).toBe(200)
      expect(fake.state.bucketObjects.get("adt-publish-snapshots")).toBeUndefined()
      expect(fake.state.buckets).toEqual([])
      expect(store.read()).toBeNull()
      expect(
        fake.state.calls.filter(
          (call) => call.method === "DELETE" && call.url.includes("/objects/"),
        ),
      ).toHaveLength(1001)
    })
