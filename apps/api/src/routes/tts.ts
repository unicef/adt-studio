...
function getSingleItemFallbackAttempts(options: {
  ...
  return attempts.filter(
    (attempt) => attempt.provider !== options.primaryProvider,
  );
}
...
export function createTTSRoutes(
  ...
  app.get("/books/:label/tts", (c) => {
    ...
    const mapEntries = (
      ...
      return entries
        .filter((entry) => readyIds.has(entry.textId))
        .map((e) => {
          let cacheKey: string | undefined;
          try {
            cacheKey = fs
              .statSync(path.join(audioDir, e.fileName))
              .mtimeMs.toString(36);
          } catch {
  ...
  app.post("/books/:label/tts/upload-one", async (c) => {
    ...
    try {
      ...
      if (existingEntry && existingEntry.fileName !== nextEntry.fileName) {
  ...
  app.post("/books/:label/tts/generate-one", async (c) => {
    ...
    try {
      ...
      const textEntry =
        entryIndex === -1 ? undefined : languageEntries[entryIndex];
      if (!textEntry) {
        throw new HTTPException(404, {
          message: `Text entry not found for ${parsed.data.textId} (${normalizedLanguage})`,
        });
      ...
      const localModelsDir =
        process.env.LOCAL_TTS_MODELS_DIR ??
        path.join(path.dirname(booksDir), ".local-models", "tts");
      const localManifest =
        provider === "local-hf"
          ? readLocalHfManifest(localModelsDir, model)
          : undefined;
      ...
      const synthesizeEntry = async (options: {
        ...
        generateSpeechFile({
          ...
          providerVariant: localManifest
            ? JSON.stringify({
                revision: localManifest.revision,
                dtype: localManifest.dtype,
                modelFile: localManifest.modelFile,
                speed: providerConfigs["local-hf"]?.speed ?? 1,
              })
            : options.targetProvider === "local-system"
              ? JSON.stringify({
                  platform: process.platform,
                  speed: providerConfigs["local-system"]?.speed ?? 1,
                })
              : undefined,
          ...
          elevenLabsPreviousText:
            options.targetProvider === "elevenlabs" &&
            config.speech?.elevenlabs_use_context
              ? findAdjacentSpeechText(
                  languageEntries,
                  entryIndex,
                  -1,
      ...
      try {
        ...
        try {
          ...
          if (fallbackModel && /did not include audio data/i.test(message)) {
            console.warn(
        ...
        const status = /\(429\)|quota|rate limit/i.test(fallbackFailureMessage)
          ? 429
          : 502;
  ...
  app.put("/books/:label/tts/timestamps/:language/:textId", async (c) => {
    ...
    try {
      const existingRow = storage.getLatestNodeData(
        "tts-timestamps",
        normalizedLanguage,
      );
  ...
  app.post("/books/:label/tts/transcribe-one", async (c) => {
    ...
    if (!openaiApiKey) {
      throw new HTTPException(400, {
        message:
          "OpenAI API key required for Whisper transcription. Set X-OpenAI-Key header.",
      });
    }
    ...
    try {
      ...
      try {
        const catalogEntries = getCatalogEntriesForLanguage(
          storage,
          sourceLanguage,
          normalizedLanguage,
        );
  ...
  app.post("/books/:label/tts/transcribe-all", async (c) => {
    ...
    const parsed = z
      .object({ language: z.string().min(1) })
      .strict()
      .safeParse(body);
    if (!parsed.success) {
      throw new HTTPException(400, {
    ...
    try {
      ...
      const existingRow = preStorage.getLatestNodeData(
        "tts-timestamps",
        normalizedLanguage,
      );
      ...
      async (emitProgress) => {
        ...
        try {
          ...
          if (toTranscribe.length === 0)
            return { count: 0, skipped: ttsEntries.length };
          ...
          for (const ttsEntry of toTranscribe) {
            try {
              ...
              const currentRow = storage.getLatestNodeData(
                "tts-timestamps",
                normalizedLanguage,
              );
            ...
            emitProgress(
              ...
              Math.round(
                ((count + newlyFailed.length) / toTranscribe.length) * 100,
              ),
            );
          }
