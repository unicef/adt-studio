...
describe("GET /p/:token/*", () => {
  ...
  /** A gated publication must never answer `public` — a shared cache in front of the worker
   *  could then hand the bytes to a request that never passed the access gate at all. This is
   *  true even for the author's own request here (authenticated with `MGMT_SECRET`, not a
   *  cookie): the policy has to reflect the publication, not how *this* caller got past the
   *  gate. */
  it("serves a gated publication's media and hashed assets as private, never public", async () => {
    const { app, env: bindings } = harness()
    await app.request(
      "/api/publications",
      mgmt(
        zipForm(
          { ...createMetadata, access_code: "SUNSET" },
          { "index.html": "<h1>cover</h1>", "images/cover.png": "png-bytes", "assets/app.a1b2c3d4e5.js": "hashed" },
        ),
      ),
      bindings,
    )
    ...
    const hashed = await app.request(
      `/p/${TOKEN}/assets/app.a1b2c3d4e5.js`,
      { headers: { Authorization: `Bearer ${SECRET}` } },
      bindings,
    )
    expect(hashed.headers.get("cache-control")).toBe("private, max-age=31536000, immutable")

    /** html was already `no-cache` — the fix does not need to, and does not, touch it. */
