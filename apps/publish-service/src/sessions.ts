...
export async function upsertCommenterSession({
  ...
  /** The read above closes the ordinary case, but not the race: two requests claiming the same
   *  name and PIN can both pass it before either has written. `idx_sessions_pinned_name_key`
   *  (migration 0008) is the backstop for that window — the loser's write throws
   *  `PinnedNameConflictError` instead of silently creating a second identity behind the same
   *  name, and the catch below turns it into the same response the read above would have given
   *  if it had run a moment later. */
  try {
    if (existing) {
    ...
    if (!(error instanceof PinnedNameConflictError)) throw error
    const taken = await pinnedHolderOf(store, token, name, existing?.id ?? null)
    return { ok: false, takenBy: taken?.name ?? error.pinnedName }
  }
}
