...
import type { AccessAttemptKind, PublicationStore, StoredCommenterSession } from "./store.js"
  ...
  return {
    ...
    async countAccessFailures({ token, client, kind, since }) {
      const recent = accessFailures.filter(
        (entry) => entry.token === token && entry.kind === kind && entry.at >= since,
      )
      return {
    ...
    async clearAccessFailures({ token, client, kind }) {
      for (let i = accessFailures.length - 1; i >= 0; i -= 1) {
        ...
        if (entry.token === token && entry.client === client && entry.kind === kind) {
          accessFailures.splice(i, 1)
        }
      }
    },
