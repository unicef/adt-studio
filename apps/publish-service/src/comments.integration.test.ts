...
describe("reviewer PINs and identity reclaim", () => {
  ...
  /** The read-then-write check above closes the ordinary case, but not two requests racing the
   *  same (name, PIN): both can pass the read before either has written. Fired together so they
   *  can actually interleave, this exercises `idx_sessions_pinned_name_key` (migration 0008) —
   *  see d1-store.integration.test.ts for a deterministic test of the index itself. Whichever
   *  request loses the race gets the same "name taken" response the read-based check produces,
   *  not a raw 500 from an uncaught constraint violation. */
  it("blocks a racing duplicate pinned name instead of creating two identities behind one name", async () => {
    const token = await publish()

    const [first, second] = await Promise.all([
      session(token, { name: "Priya", pin: "2468" }),
      session(token, { name: "Priya", pin: "2468" }),
    ])
    ...
    await expect(loser.json()).resolves.toMatchObject({ error: "name_taken" })

    const row = await env.DB.prepare(
      `SELECT COUNT(*) AS total FROM sessions WHERE token = ? AND is_author = 0`,
    )
      .bind(token)
      .first<{ total: number }>()
...
describe("guessing a reviewer's PIN", () => {
  ...
  /** The PIN door's own limit trips on the PIN door's own failures — access-throttle.test.ts
   *  and access.integration.test.ts's "does not reset the PIN door's counter" cover the other
   *  half: a correct answer at the access-code door leaves this count untouched too. */
  it("keeps counting failures against the PIN door's own limit", async () => {
    const token = await publish()
    await session(token, { name: "Maria", pin: "2468" })
