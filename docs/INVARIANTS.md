...
| 8 | B1 mode transitions retain entities/media and gate persisted Storyboard input | `apps/api/src/services/sectioning-lifecycle.test.ts`, `sectioning-preflight-runners.test.ts`, `packages/pipeline/src/__tests__/sectioning-preflight.test.ts`, and `packages/storage/src/__tests__/book-writer.test.ts` under `pnpm test` | every PR | SPEC-0003 B1 implementation (review pending) |
