...
Full-screen pages navigated with ← → (capture-phase handler intercepts the book's own arrow navigation; inputs guarded via `isTypingTarget`). Sequence: **welcome → how-do-you-want-to-read (read-mode choice first, per the ADT demo; sets `readAloudModeAtom`) → your name → pick buddy (speech bubble with randomized per-character phrases; names shown, not species) → "Turn the pages" → "Ask me anytime" (L key) → "Here's what I can do" (feature-gated ability list) → start**. When read-aloud is on, each step is read aloud by the neutral narrator with a visible play/replay button; "I'll read it myself" keeps the flow silent (no autoplay, no button). Gated by `kidsOnboardingDone`.

### Studio authoring screen (Kids Mode)

The `/books/:label/kids` screen is a canvas+inspector authoring tool: a persistent buddy avatar rail, a center stage with **Character art** / **Lines** tabs (grow-to-fit expression preview + horizontal expression row; the audition lines list), and a right voice inspector (editable preset + prompt, save/reset, per-buddy generation). A collapsible "what is Kids Mode" explainer (buddy cluster + feature cards) sits on top, expanded by default. Studio buddy thumbnails live at `apps/studio/src/assets/kids-buddies/<id>/<id>_1..7.png` (copies of the runtime art, byte-pinned by `kids-buddies-sync.test.ts` since the layer rule forbids cross-app imports).
