...
import { readableTextColor } from "../commenter-color.js"
import {
  COMMENTER_COLORS,
  CommentAnchor,
  PublishComment,
...
describe("readableTextColor", () => {
  /** Ported from the Studio's copy when the function was hoisted here. The two cases it named
   *  are the ones that motivated it: amber is light enough that white fails, indigo is not. */
  it("picks dark text on the light rotation colours and white on the dark ones", () => {
    expect(readableTextColor("#ffb224")).toBe("#1a1a1a")
    expect(readableTextColor("#3e63dd")).toBe("#ffffff")
  })

  /** The rotation is the only input this is tuned against, so every entry gets an answer —
   *  a colour added to `COMMENTER_COLORS` without a contrast decision fails here. */
  it("answers for every colour in the rotation", () => {
    for (const colour of COMMENTER_COLORS) {
      expect(["#1a1a1a", "#ffffff"]).toContain(readableTextColor(colour))
    }
  })
