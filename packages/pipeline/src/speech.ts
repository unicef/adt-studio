 ...
 */
export function classifyElevenLabsTtsError(
  message: string,
): ElevenLabsTtsErrorKind {
  const status = parseElevenLabsErrorStatus(message);
  if (status === 429) return "rate-limit";
  if (status !== null)
    return status >= 500 && status <= 599 ? "transient" : "permanent";
...
export function computeSpeechCacheKey(
  data: {
    text: string;
    voice: string;
    model: string;
...
export function buildElevenLabsTtsLogParams(
  ...
  return {
    ...
    ...(input.previousText
      ? { contextBeforeChars: input.previousText.length }
      : {}),
...
export async function generatePageSpeechFiles(
  ...
  if (safeFormat !== "wav") {
    throw new Error(
      `Page-batched TTS requires wav output; received: ${format}`,
    );
  }
  const normalizedLanguage = assertSafeSegment(
    normalizeLocale(language),
  ...
  const pageCachePath = path.resolve(
    pageCacheRoot,
    `${pageHash}.${safeFormat}`,
  );
  ...
  for (let k = 1; k < ranges.length; k++) {
    const snapped = findQuietCutSeconds(
      pageBytes,
      ranges[k].start,
      PAGE_SLICE_SNAP_WINDOW_SEC,
    );
    const bounded = Math.min(
      Math.max(snapped, ranges[k - 1].start),
      totalDuration,
    );
  ...
  for (const range of ranges) {
    ...
    if (
      ...
      !fs.readFileSync(outputPath).equals(slice)
    ) {
