...
describe("web rendering reading-order prompts", () => {
  ...
  it("preserves separated labels as an accessible labeled diagram", async () => {
    ...
    expect(prompt).toContain("Required LEFT-side structure example")
    ...
    expect(prompt).toContain("SAME positioned callout item")
    expect(prompt).toContain('data-diagram-callout class="... flex items-center gap-0')
    expect(prompt).toContain('data-label-contact="start"')
    expect(prompt).toContain("underlap is masked at the label edge")
    expect(prompt).toContain("Never add separate `h-px`")
  ...
  it("requires visual review to reject labels lost during image extraction", async () => {
    ...
    expect(prompt).toContain("reject any whitespace gap between the label container")
    expect(prompt).toContain("a line crossing through label glyphs")
    expect(prompt).toContain("a label with no corresponding leader")
    expect(prompt).toContain("zero-gap siblings in the same positioned callout item")
    expect(prompt).toContain("Check this at desktop and tablet widths")
