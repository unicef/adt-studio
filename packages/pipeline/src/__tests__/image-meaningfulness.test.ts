...
describe("buildMeaningfulnessConfig", () => {
  ...
  it("keeps optional meaningfulness filtering off for embedded defaults", () => {
    ...
    expect(buildMeaningfulnessConfig(appConfig)?.modelId).toBe(
      "local:gemma4-12b",
    );
  });
  ...
  it("passes the explicit auto figure extraction mode to the prompt config", () => {
    ...
    expect(buildMeaningfulnessConfig(appConfig)?.figureExtractionMode).toBe(
      "auto",
    );
  });
});
      ...
      groups: [
        {
          ...
          shapes: [
            ...
            {
              type: "image" as const,
              bbox: [20, 0, 40, 20] as [number, number, number, number],
            },
          ],
...
describe("deduplicateAutoFigureCandidates", () => {
  ...
  it("keeps the standalone when the composite was already pruned", () => {
    ...
    expect(deduplicateAutoFigureCandidates(classification, debug)).toBe(
      classification,
    );
  });
});
...
describe("filterPageImageMeaningfulness", () => {
  ...
  it("returns existing classification when no images", async () => {
    const existing: ImageClassificationOutput = {
      images: [{ imageId: "pg001_im001", isPruned: true, reason: "too small" }],
    };
  ...
  it("uses a transparent Auto reason when a candidate is better represented as HTML", async () => {
    ...
    expect(result.images[0]).toEqual({
      ...
      isPruned: true,
      reason:
        "auto figure filter: Selectable heading text in a simple colored rectangle",
    });
  });
  ...
  it("protects Auto candidates without selectable-text evidence", async () => {
    ...
    expect(result.images[0]).toEqual({
      ...
      isPruned: false,
      reason:
        "auto guard kept: raster artwork with no selectable-text copy and no surviving standalone duplicate (LLM: Looks text-heavy)",
    });
  });
    ...
    expect(result.images).toEqual([
      {
        ...
        isPruned: true,
        reason:
          "auto figure filter: The crop only adds a heading around the photo",
      },
      { imageId: "pg001_im002", isPruned: false },
    ]);
  });
    ...
    expect(result.images).toEqual([
      {
        ...
        isPruned: false,
        reason:
          "auto guard kept: raster artwork with no selectable-text copy and no surviving standalone duplicate (LLM: Looks like a text-heavy scan)",
      },
      {
        imageId: "pg001_im002",
        isPruned: true,
